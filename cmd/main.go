...
import (
	...
	"fluid/agents/gitlab/internal/agent"
...
func main() {
	...
	credentialsPath := flag.String("credentials", "/etc/fluid/gitlab/credentials.yaml", "Durable credentials (organization_uuid + connection token)")
	enrollmentEnvPath := flag.String("enrollment-env", "/etc/fluid/gitlab/enrollment.env", "systemd EnvironmentFile path removed after enrollment when FLUID_ENROLL_PURGE_ENROLLMENT_SOURCES is true")
	flag.Parse()
	...
	if p := strings.TrimSpace(os.Getenv("FLUID_SERVICE_CREDENTIALS_ENV_FILE")); p != "" {
		if err := config.MergeDotenvIntoGitLab(cfg, p); err != nil {
			log.Fatalf("service credentials env file: %v", err)
		}
		log.Printf("merged GitLab token from FLUID_SERVICE_CREDENTIALS_ENV_FILE=%q", p)
	}
	...
	if strings.TrimSpace(cfg.Controlplane.Token) == "" && enrollSecret != "" {
		...
		enrollName := enroll.ResolveExecutionAgentEnrollmentName(hostname, "gitlab", override)
	...
	if err != nil {
		log.Fatalf("failed to initialize execution agent: %v", err)
	}

	if err := execAgent.Start(); err != nil {
		log.Fatalf("failed to start execution agent: %v", err)
	}
	log.Printf("agent process is running, waiting for signals")
